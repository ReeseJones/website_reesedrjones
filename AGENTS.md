...
## Commands
- `npm run start` — Local dev server with live reload / HMR (cleans `dist`)
- `npm run build` — Production build to `./dist`
- `npm run preview` — Build and run local Cloudflare preview (`wrangler dev`)
- `npm run deploy` — Build and deploy directly to Cloudflare (`wrangler deploy`)
...
- **Parcel cache:** Delete `.parcel-cache` and `dist` if Parcel hits stale build errors.
